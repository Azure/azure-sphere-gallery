...
EventLoopTimer *CreateEventLoopPeriodicTimer(EventLoop *eventLoop, EventLoopTimerHandler handler,
                                             const struct timespec *period, void *context)
{
    if (handler == NULL) {
        errno = EINVAL;
        return NULL;
    ...
    EventLoopTimer *timer = (EventLoopTimer*)malloc(sizeof(EventLoopTimer));
    if (timer == NULL) {
        return NULL;
    }
...
EventLoopTimer *CreateEventLoopDisarmedTimer(EventLoop *eventLoop, EventLoopTimerHandler handler, void *context)
{
    return CreateEventLoopPeriodicTimer(eventLoop, handler, NULL, context);
}
