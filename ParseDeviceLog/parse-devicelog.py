...
"""
This module parses Azure Sphere device binary log file into a human-readable format.
The module requires two command line arguments:
- an Azure Sphere OS Version (Example: 22.09)
- and a device log binary file
The device log binary file should be extracted from the zip file generated by the following Azure Sphere CLI command:
'azsphere get-support-data'
"""
...
class LogParser:
    ...
    def __init__(self, os_version):
        ...
        """ 
        manifest_dir_path = os.path.join(os.path.dirname(__file__), os_version, "log_manifests")
        yaml_dir_path = os.path.join(os_version, "commonerror_yml")

        error_code.set_error_yml_path(yaml_dir_path)
        ...
        if  os.path.isdir(manifest_dir_path):            
            ...
            print("manifest directory path doesn't exist")
            exit(1)
...
if __name__=='__main__':
    ...
    file_content = ""
    if not os.path.isfile(args.bin_filename):
        print(f"File '{args.bin_filename}' does not exist")
        exit(1)
