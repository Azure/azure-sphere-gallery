...
static int json_append_to_path(struct frozen *f, const char *str, int size) {
  ...
  if (size > 0)
  {
      memcpy_s(f->path + n, left, str, size);
      f->path[n + size] = '\0';
      f->path_len += size;
  }
  return n;
...
int json_printer_buf(struct json_out *out, const char *buf, size_t len) {
  ...
  if (n > 0)
  {
    memcpy_s(out->u.buf.buf + out->u.buf.len, avail, buf, n);
    out->u.buf.len += n;
  }
  if (out->u.buf.size > 0) {
  ...
  while (*fmt != '\0') {
    if (strchr(":, \r\n\t[]{}\"", *fmt) != NULL) {
      ...
      if (fmt[1] == 'l' && fmt[2] == 'l' && (fmt[3] == 'd' || fmt[3] == 'u')) {
        ...
        char fmt2[21];
        va_list ap_copy;
        strncpy_s(fmt2, sizeof(fmt2), fmt,
                n + 1 > (int) sizeof(fmt2) - 1 ? sizeof(fmt2) - 1 : (size_t) n + 1);
        // fmt2[n + 1] = '\0';
...
int json_printf_array(struct json_out *out, va_list *ap) {
  ...
  for (i = 0; arr != NULL && i < arr_size / elem_size; i++) {
    ...
    int n = elem_size > sizeof(val) ? sizeof(val) : elem_size;
    memcpy_s(&val, sizeof(val), arr + i * elem_size, n);
...
static void json_scanf_cb(void *callback_data, const char *name,
  ...
  switch (info->type) {
    ...
    default:
      ...
      memcpy_s(buf, sizeof(buf), token->ptr, token->len);
      buf[token->len] = '\0';
...
int json_vscanf(const char *s, int len, const char *fmt, va_list ap) {
  ...
  while (fmt[i] != '\0') {
    if (fmt[i] == '{') {
      strcat_s(path, JSON_MAX_PATH_LEN, ".");
      i++;
    } else if (fmt[i] == '}') {
      ...
      switch (fmt[i + 1]) {
        ...
        default: {
          ...
          memcpy_s(fmtbuf, sizeof(fmtbuf), fmt + i, conv_len);
          fmtbuf[conv_len] = '\0';
          i += conv_len;
      ...
      pe = path + path_len;
      memcpy_s(pe, sizeof(path) - path_len, fmt + i, key_len);
      pe[key_len] = '\0';
...
static int json_sprinter(struct json_out *out, const char *str, size_t len) {
  ...
  if (p != NULL) {
    memcpy_s(p + old_len, len + 1, str, len);
    p[new_len] = '\0';
    out->u.buf.buf = p;
  }
  return len;
...
static int json_sprinter(struct json_out *out, const char *str, size_t len) {
  size_t old_len = out->u.buf.len;
  size_t new_len = len + old_len;
  char *p = out->u.buf.buf;

  if (new_len >= out->u.buf.size) {

    if (!out->u.buf.size) {
      out->u.buf.size = new_len + 1;
    } else {
      while (new_len >= out->u.buf.size)  out->u.buf.size *= 2;
    }
    p = (char*)REALLOC(out->u.buf.buf, out->u.buf.size);    
  }


  if (p != NULL) {
    memcpy_s(p + old_len, out->u.buf.size - old_len, str, len);
    p[new_len] = '\0';
    out->u.buf.buf = p;
